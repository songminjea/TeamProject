...
<body topmargin="0" leftmargin="0" rightmargin="0" bottommargin="0">
	...
	<div id="positionLayer">
		...
		<div>
			<%-- 페이징 --%>
			<ul class="pagination">
				<c:if test="${chatPageMaker.prev}">
					<li>
						<a href="${pageContext.request.contextPath}/${member.ID}/chatSearchList${chatPageMaker.makeSearch(pageMaker.startPage-1)}">
							<i class="fa fa-chevron-left" aria-hidden="true"></i>
						</a>
					</li>
				</c:if>
				<c:forEach begin="${chatPageMaker.startPage}" end="${chatPageMaker.endPage}" var="idx">
					<li <c:out value="${chatPageMaker.cri.page==idx?'class=active' : ''}"/>>
						<a href="${pageContext.request.contextPath}/${member.ID}/chatSearchList${chatPageMaker.makeSearch(idx)}">${idx}</a>
					</li>
				</c:forEach>
				<c:if test="${chatPageMaker.next && chatPageMaker.endPage > 0}">
					<li>
						<a href="${pageContext.request.contextPath}/${member.ID}/chatSearchList${chatPageMaker.makeSearch(chatPageMaker.endPage + 1)}">
